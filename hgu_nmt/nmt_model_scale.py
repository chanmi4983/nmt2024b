...
def get_scale(eps, n1, n2=None): # for Xavier uniform initialization
...
class myEmbedding(nn.Embedding):
    ...
    def reset_parameters(self):
        scale = get_scale(1,1,self.embedding_dim)
...
class ScaledDotProductAttention(nn.Module): 
    def __init__(self, dk, drop_p=0.):
        ...
        self.temper = dk #** 0.5 ###float(dk)# ** 0.5 # this is better without 0.5
...
class MultiHeadAttention(nn.Module):
    def __init__(self, args, n_head, dim_model, dk, dv, drop_p=0.):
...
def forward(self, q, k, v, attn_mask=None): # (QK')V # q = B T E
    ...
    # pre-LN 처리
    if self.args.norm_case == 'prenorm':
        qnew = self.layer_norm(q)
        k = self.layer_norm(k)
        v = self.layer_norm(v)
    else:
        qnew = q  # pre-LN이 아닐 경우, 원본 q 사용
    ...
    # norm_case에 따른 처리
    if self.args.norm_case == 'postnorm':
        return self.layer_norm(output + q)

    elif self.args.norm_case == 'ours':
        return self.layer_norm(output) + q

    elif self.args.norm_case == 'prenorm':
        return output + q  # pre-LN

class FeedForward(nn.Module):
    def __init__(self, args, dim_model, dim_ff, drop_p=0.):
        super().__init__()
    ...
    def forward(self, x):
        # pre-LN 처리
        if self.args.norm_case == 'prenorm':
            xnew = self.layer_norm(x)  # prenorm인 경우 Layer Norm 적용
        else:
            xnew = x  # prenorm이 아닐 경우 원본 x 사용
        ...
        # norm_case에 따른 결과 반환
        if self.args.norm_case == 'postnorm':
            return self.layer_norm(output + x)  # Post-LN

        elif self.args.norm_case == 'ours':
            return self.layer_norm(output) + x  # Custom-LN

        elif self.args.norm_case == 'prenorm':
            return output + x  # Pre-LN

class PositionalEncoding(nn.Module):
    def __init__(self, eps,d_model, dropout=0.1):
        super(PositionalEncoding, self).__init__()
        self.dropout = nn.Dropout(p=dropout)
        self.d_model = d_model
        self.max_len = 200
        self.get_pe(eps,self.max_len)
    ...
    def get_pe(self, eps, max_len):
        scale = get_scale(eps, self.d_model)
...
class TM_EncoderLayer(nn.Module):
    def __init__(self, args,  dim_model, dim_ff, n_head, dk, dv, drop_p=0.):
        super(TM_EncoderLayer, self).__init__()
        self.self_attn = MultiHeadAttention(args, n_head, dim_model, dk, dv, drop_p=drop_p)
        self.ff_layer = FeedForward(args,dim_model, dim_ff, drop_p=drop_p)
...
class TM_Encoder(nn.Module):
    def __init__(self, args, src_words_n, n_layers=6, n_head=8, dk=64, dv=64,
        ...
        self.pos_enc = PositionalEncoding(args.eps,dim_wemb)
        self.layer_norm = nn.LayerNorm(dim_wemb)
        self.args = args
        # repeated layer
        self.layer_stack = nn.ModuleList([
            TM_EncoderLayer(args,dim_model, dim_ff, n_head, dk, dv, drop_p=drop_p)
    ...
    def forward(self, src_seq, src_mask):
        ...
        #if self.args.norm_case == 'prenorm' or self.args.norm_case == 'ours':
        #    enc_out = self.layer_norm(enc_out) #only uncomment for pre and post

        for enc_layer in self.layer_stack:
            enc_out = enc_layer(enc_out, x_mask=src_mask)
        
        #if self.args.norm_case == 'postnorm' or self.args.norm_case == 'ours':
        return enc_out
        #elif self.args.norm_case == 'prenorm':
        #    return self.layer_norm(enc_out) #prenorm

class TM_DecoderLayer(nn.Module):
    def __init__(self,args, dim_model, dim_ff, n_head, dk, dv, drop_p=0.):
        super(TM_DecoderLayer, self).__init__()
        self.self_attn = MultiHeadAttention(args,n_head, dim_model, dk, dv, drop_p=drop_p)
        self.enc_attn = MultiHeadAttention(args,n_head, dim_model, dk, dv, drop_p=drop_p)
        self.ff_layer = FeedForward(args,dim_model, dim_ff, drop_p=drop_p)
...
class TM_Decoder(nn.Module):
    def __init__(self, args, trg_words_n, n_layers=6, n_head=8, dk=64, dv=64,
            dim_wemb=512, dim_model=512, dim_ff=1024, drop_p=0., emb_noise=0.):
        ...
        # repeated layer
        self.layer_stack = nn.ModuleList([
            TM_DecoderLayer(args, dim_model, dim_ff, n_head, dk, dv, drop_p=drop_p)
            for _ in range(n_layers)])
        # readout 
...
class Transformer(nn.Module):
    def __init__(self, args=None):
        ...
        self.encoder=TM_Encoder(args, src_words_n, n_layers=n_layers, n_head=n_head,
                dim_wemb=dim_model, dim_model=dim_model, dim_ff=dim_ff, drop_p=drop_p, 
                emb_noise=args.emb_noise)
        self.decoder=TM_Decoder(args, trg_words_n, n_layers=n_layers, n_head=n_head,
                dim_wemb=dim_model, dim_model=dim_model, dim_ff=dim_ff, drop_p=drop_p, 
                emb_noise=args.emb_noise)
