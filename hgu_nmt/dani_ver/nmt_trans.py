...
def translate_tm2(model, x_data, x_mask, args):
    ...
    for yi in range(args.max_length):
        ...
        dec_in = model.dec_emb(dec_seq) # Word embedding look up # Ty B E 
...
def translate_tm_beam(model, x_data, x_mask, args): # not working
    ...
    for len_dec_seq in range(1, args.max_length + 1):
        ...
        print('activ', active_inst_idx_list)
...
def translate_rnnnmt(model, x_data, args):
    # encoding
...
def translate_file(model, args, valid=None):
    ...
    valid_iter = TextIterator(args.valid_src_file, args.src_dict, batch_size=100, 
                    maxlen=args.max_length, ahead=1, resume_num=0, just_one_epoch=1, 
                    const_id=Const)
    ...
    with torch.no_grad():
        for x_data, x_mask, cur_line, iloop in valid_iter:
            if args.model == 'tm':
                samples = translate_tm(model, x_data, x_mask, args)
            elif args.model == 'tm2':
                samples = translate_tm2(model, x_data, x_mask, args)
            else:
                samples = translate_rnnnmt(model, x_data, args)
