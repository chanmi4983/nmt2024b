...
class ScaledDotProductAttention(nn.Module): 
    def __init__(self, dk, drop_p=0.):
        ...
        self.temp_const = float(dk) # ** 0.5 # TEST it was better without 0.5 (layer_norm, init)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_mask=None): # (QK')V # q = B T E
        ...
        return self.layer_norm(output) + q # Different from Transformer 2017
...
class MultiHeadAttentionNew(nn.Module):
    def __init__(self, n_head, dim_model, dk, dv, drop_p=0.):
        ...
        self.mat_ks = myLinear(dim_model, dim_model, bias=False)
        self.mat_vs = myLinear(dim_model, n_head*dv, bias=False)
    ...
    def forward(self, q, k, v, attn_mask=None): # (QK')V # q = B T E
        ...
        # TODO: merge the matrices and project q,k,v at the same time and split
        qnew = q.view(Bn, Tq, n_head, -1).transpose(1,2)
        k = self.mat_ks(k).view(Bn, Tk, n_head, -1).transpose(1,2)
        #k = k.view(Bn, Tk, n_head, -1).transpose(1,2)
        v = self.mat_vs(v).view(Bn, Tv, n_head, dv).transpose(1,2)
...
class MultiHeadAttentionCross(nn.Module):
    def __init__(self, n_head, dim_model, dk, dv, drop_p=0.):
        super(MultiHeadAttentionCross, self).__init__()

        self.n_head, self.dk, self.dv = n_head, dk, dv

        self.mat_qs = nn.Linear(dim_model, n_head*dk, bias=False) 
        self.mat_ks = nn.Linear(dim_model, n_head*dk, bias=False)

        self.sdp_attn = ScaledDotProductAttention(dk, drop_p=drop_p)

        self.out_proj = nn.Linear(n_head*dv, dim_model)
        self.dropout = nn.Dropout(p=drop_p)
        self.layer_norm = nn.LayerNorm(dim_model)

    def forward(self, q, k, v, attn_mask=None): # (QK')V # q = B T E, (q=Dec, k=Enc, v=Enc)
        n_head, dk, dv = self.n_head, self.dk, self.dv
        Bn, Tq, Tk, Tv = q.size(0), q.size(1), k.size(1), v.size(1)
        ...
        qnew = self.mat_qs(q).view(Bn, Tq, n_head, dk).transpose(1,2)
        k = self.mat_ks(k)
        #v = self.mat_vs(k).view(Bn, Tv, n_head, dv).transpose(1,2)
        k = k.view(Bn, Tk, n_head, dk).transpose(1,2)
        v = k
...
class MultiHeadAttentionSelf(nn.Module):
    def __init__(self, n_head, dim_model, dk, dv, drop_p=0.):
        super(MultiHeadAttentionSelf, self).__init__()

        self.n_head, self.dk, self.dv = n_head, dk, dv

        self.mat_qs = nn.Linear(n_head*dv, n_head*dk, bias=False) 
        self.mat_vs = nn.Linear(dim_model, n_head*dv, bias=False)
    ...
    def forward(self, q, k, v, attn_mask=None): # (QK')V # q = B T E
        ...
        # TODO: merge the matrices and project q,k,v at the same time and split
        vnew = self.mat_vs(v)
        qnew = self.mat_qs(vnew).view(Bn, Tq, n_head, dk).transpose(1,2)
        #k = self.mat_ks(vnew).view(Bn, Tk, n_head, dk).transpose(1,2)
        vnew = vnew.view(Bn, Tv, n_head, dv).transpose(1,2)
        k = vnew

        if attn_mask is not None: #  B ? T -> B ? ? T 
            attn_mask = attn_mask.unsqueeze(1)
        
        output = self.sdp_attn(qnew, k, vnew, mask=attn_mask) # Bn H Ty E
        output = output.transpose(1, 2).contiguous().view(Bn, Tq, -1) # Bn Ty H*E
        output = self.out_proj(output)
        output = self.dropout(output)
...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.dropout(output)
        return self.layer_norm(output) + x # Different from Transformer 2017
...
class TM_Decoder(nn.Module):
    def __init__(self, trg_words_n, n_layers=6, n_head=8, dk=64, dv=64,
        ...
        # first in 
        self.trg_emb = myEmbedding(trg_words_n, dim_wemb)#, padding_idx=Const.PAD)
        self.pos_enc = PositionalEncoding(dim_wemb)
        self.emb_dropout = nn.Dropout(p=drop_p)
        self.layer_norm = nn.LayerNorm(dim_wemb)
        ...
        #self.trg_word_proj.weight = self.trg_emb.weight # Share the weight 
    ...
    def forward(self, trg_seq, trg_mask, enc_out, src_mask):
        ...
        dec_out = self.trg_emb(trg_seq) # Word embedding look up
