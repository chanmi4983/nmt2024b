...
VALID_FILE1=$DATA_DIR'/aihub.valid.'$LANG1'.tok'$SYMBOL
VALID_FILE2=$DATA_DIR'/aihub.valid.'$LANG2'.tok'$SYMBOL
...
MODEL_FILE=$LANG1'2'$LANG2'PostLN2.gpu'$1'.demo'
# 모델이름 바꾸기

RESUME=0 #RESUME 할 경우 1
OPT_SCHEDULED=1
RESUME_LINE=0 #2850000 #191360000 # The last iteration when RESUME=1 #직접 계산해서 int형으로 넣기
...
#RESUME_FILE=en2krContextTest_case3_1.gpu5.demo.pth # space 없이 붙여서
...
CUDA_VISIBLE_DEVICES=$1,CUDA_LAUNCH_BLOCKING=1 python3 -W ignore nmt_train.py \
        ...
        --n_layers=$N_LAYERS --dim_ff=$DIM_FF --batch_size=$BATCH_SIZE \
