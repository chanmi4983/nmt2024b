...
class MultiHeadAttention(nn.Module):
    def __init__(self, n_head, dim_model, dk, dv, drop_p=0.):
        ...
        # (layer_id, out_n, ratio) or (layer_id, res_n, br_n, out_n, ratio) 등
        # 너의 파이프라인에 맞춰 append하는 포맷을 아래에서 선택 가능
        self.res_ratio_log = []
        self.layer_id = "UNK"

        # ===== (추가) step별 상세 디버깅 로깅 =====
        # step마다 토큰별 norm/ratio가 어떻게 변하는지 확인용
        self.collect_step_detail = False
        # item: (layer_id, step_T, res_norm[T], br_norm[T], ratio_tok[T], sent_ratio)
        # 여기서 sent_ratio는 "토큰별 ratio 평균" (mask 반영)
        self.step_detail_log = []
    ...
    def forward(self, q, k, v, attn_mask=None, token_mask=None):
        ...
        branch = self.layer_norm(output)  # add 직전 branch
        residual = q                      # add 직전 residual

        # ===== add 직전에서 토큰별 norm/ratio 계산 =====
        eps = 1e-12
        res_norm = residual.norm(dim=-1)           # (B,T)
        br_norm  = branch.norm(dim=-1)             # (B,T)
        ratio_tok = br_norm / (res_norm + eps)     # (B,T)

        # token_mask 반영하여 "문장(현재 prefix) 평균 ratio" 계산
        if token_mask is None:
            sent_ratio = ratio_tok.mean(dim=1)  # (B,)
            sent_n = torch.full((ratio_tok.size(0),), ratio_tok.size(1), device=ratio_tok.device)
        else:
            m = token_mask.float()
        ...
        if self.collect_res_ratio:
            # 네가 현재 write_layerwise_resratio_mean에서 3개 포맷을 쓰고 있다면 이 포맷 추천:
            # (layer_id, out_n, ratio)
            for r, n_used in zip(sent_ratio.detach().cpu().tolist(),
                                 sent_n.detach().cpu().tolist()):
                self.res_ratio_log.append((self.layer_id, int(n_used), float(r)))

            # 만약 예전 5개 포맷을 그대로 유지해야 하면 위 블록 대신 아래를 사용:
            # sent_res = (res_norm*m).sum/denom 같은 것도 계산해서 넣어야 하므로
            # 지금은 3개 포맷으로 통일하는 쪽이 안전함.

        # ===== (추가) step별 상세 디버깅 로그 =====
        if self.collect_step_detail:
            # step_T = 현재 prefix 길이(T). 디코더에서는 step마다 1,2,3,...로 변함
            step_T = int(ratio_tok.size(1))

            # 배치가 1이 아닐 수도 있으니, batch 전체를 저장하면 너무 커져서
            # 일단 batch=1 기준으로 첫 샘플만 저장 (원하면 for-loop로 B 전체 저장도 가능)
            self.step_detail_log.append((
                self.layer_id,
                step_T,
                res_norm[0].detach().cpu(),   # (T,)
                br_norm[0].detach().cpu(),    # (T,)
                ratio_tok[0].detach().cpu(),  # (T,)
                float(sent_ratio[0].item())
            ))
...
class FeedForward(nn.Module):
    ...
    def forward(self, x, token_mask=None):
        ...
        # token_mask 반영한 "문장(현재 prefix) 평균 ratio"
        if token_mask is None:
            sent_ratio = ratio_tok.mean(dim=1)  # (B,)
            sent_n = torch.full((ratio_tok.size(0),), ratio_tok.size(1), device=ratio_tok.device)
        else:
        ...
        if self.collect_step_detail:
            ...
            self.step_detail_log.append((
                self.layer_id,
                step_T,
                res_norm[0].detach().cpu(),
                br_norm[0].detach().cpu(),
                ratio_tok[0].detach().cpu(),
                float(sent_ratio[0].item())
            ))
