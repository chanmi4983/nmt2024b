...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_mask=None):
        ...
        Bn, Tq, Tk, Tv = q_norm.size(0), q_norm.size(1), k_norm.size(1), v_norm.size(1)
         
        qnew = self.mat_qs(q_norm).view(Bn, Tq, n_head, dk).transpose(1,2)
        knew = self.mat_ks(k_norm).view(Bn, Tk, n_head, dk).transpose(1,2)
        vnew = self.mat_vs(v_norm).view(Bn, Tv, n_head, dv).transpose(1,2)
         
        if attn_mask is not None:
            attn_mask = attn_mask.unsqueeze(1)
            
        output = self.sdp_attn(qnew, knew, vnew, mask=attn_mask)
...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        xnew = self.layer_norm(x)  # xnew = self.layer_norm(x)
