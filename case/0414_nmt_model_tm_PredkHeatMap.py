...
def calculate_entropy(attn, mask):
    """
    엔트로피를 계산하는 함수.

    Parameters:
        attn (torch.Tensor): Attention 행렬, shape (B, H, Tq, Tk).
        mask (torch.Tensor): 마스크 텐서, shape (B, 1, Tq, Tk).
        entropy_param (float): 엔트로피 조정 파라미터.

    Returns:
        torch.Tensor: 엔트로피 값, shape (B, Tq).
    """
    #print(f"Initial attn shape: {attn.shape}") #(B, H, Tq, Tk)
    #print(f"Initial mask shape: {mask.shape}") #(B, 1, Tq, Tk)
    # 마스크 적용: log 계산의 안정성을 위해 mask로 지정된 부분에 작은 값을 추가
    masked_attn = attn.clone()
    masked_attn = masked_attn.masked_fill(mask < 0.1, 1e-9)

    # 엔트로피 계산: e = -entropy_param * attn * log(attn) #(B, H, Tq, Tk)
    entropy = -masked_attn * torch.log(masked_attn + 1e-9)
    #print(f"Entropy shape after log: {entropy.shape}") 

    # 차원 합산 및 마스크 적용
    entropy = entropy * mask  # 엔트로피에 마스크 적용 #(B, H, Tq, Tk)
    #print(f"Entropy shape after mask: {entropy.shape}") 
    entropy_sum = entropy.sum(dim=-1)  # Tk 차원 합산
    #print(f"Entropy sum shape: {entropy_sum.shape}") #(B, H, Tq, Tk) -> (B, H, Tq)
    mask_sum = mask.sum(dim=-1) #+ 1e-9  # 마스크된 유효 값 개수 #(B, 1, Tq, Tk) -> (B, 1, Tq)
    #print(f"Mask sum shape: {mask_sum.shape}")

    # 마스크 기반 평균 계산: sum(e * m) / sum(m) #(B, H, Tq) / (B, 1, Tq)
    entropy_avg = entropy_sum / mask_sum
    #print(f"Entropy avg shape: {entropy_avg.shape}")
    e = entropy_avg.mean() 
    #print(f"Final entropy (scalar): {e}") # (B, H, Tq) -> (B, H) -> scalar
    #return e
    return e.detach().cpu().item() #동녕님이 용량 해결해주신 코드


'''
def calculate_entropy(attn, mask):
    print(f"Initial attn shape: {attn.shape}")
    e = -attn * torch.log(attn)
    print(f"Entropy shape after log: {e.shape}")
    e = e.sum(dim=-1)
    print(f"Entropy shape after sum(dim=-1): {e.shape}")
    print(f"Mask shape : {mask.shape}")
    e = (e * mask).sum(dim=-1) / mask.sum(dim=-1)
    print(f"Entropy shape after mask: {e.shape}")
    e = e.mean()
    print(f"Final entropy (scalar): {e}")
    return e
'''
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, attn_mask=None):
        ...
        k = self.layer_norm(k)
        v = self.layer_norm(v)
        # print(f"Before layer_norm q type: {type(q)}")
        # qnew = self.layer_norm(q)
        # print(f"After layer_norm q type: {type(qnew)}")
        
        # print(f"Before layer_norm k type: {type(k)}")
        # k = self.layer_norm(k)
        # print(f"After layer_norm k type: {type(k)}")
        
        # print(f"Before layer_norm v type: {type(v)}")
        # v = self.layer_norm(v)
        # print(f"After layer_norm v type: {type(v)}")
        ...
        output, entropy, attn= self.sdp_attn(qnew, k, v, mask=attn_mask)
        output = output.transpose(1, 2).contiguous().view(Bn, Tq, -1)
        output = self.out_proj(output)
        output = self.dropout(output)
        
        return output + q , entropy, attn
...
class FeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        output = self.layer2(self.dropout1(F.relu(self.layer1(xnew))))
...
class TM_EncoderLayer(nn.Module):
    ...
    def forward(self, enc_in, x_mask=None):
        enc_out, enc_entropy, attn = self.self_attn(enc_in, enc_in, enc_in, attn_mask=x_mask)
        enc_out = self.ff_layer(enc_out)
        return enc_out, enc_entropy, attn
...
class TM_Encoder(nn.Module):
    ...
    def forward(self, src_seq, src_mask):
        ...
        for enc_layer in self.layer_stack:
            enc_out, enc_entropy , attn= enc_layer(enc_out, x_mask=src_mask)
            enc_entropy_list.append(enc_entropy)
            enc_attn_list.append(attn)
        return enc_out, enc_entropy_list, enc_attn_list
...
class TM_DecoderLayer(nn.Module):
    ...
    def forward(self, dec_in, enc_out, y_mask=None, enc_mask=None): 
        dec_out , self_entropy, self_attn = self.self_attn(dec_in, dec_in, dec_in, attn_mask=y_mask)

        dec_out , cross_entropy, cross_attn = self.enc_attn(dec_out, enc_out, enc_out, attn_mask=enc_mask)
        
        dec_out = self.ff_layer(dec_out)
        return dec_out,self_entropy, cross_entropy, self_attn, cross_attn
...
class TM_Decoder(nn.Module):
    ...
    def forward(self, trg_seq, trg_mask, enc_out, src_mask):
        ...
        for dec_layer in self.layer_stack:
            dec_out,self_entropy, cross_entropy ,self_attn,cross_attn= dec_layer(dec_out, enc_out, y_mask=y_mask, enc_mask=src_mask)
            self_entropy_list.append(self_entropy)
        ...
        return dec_out,self_entropy_list, cross_entropy_list,self_attn_list,cross_attn_list
...
class Transformer(nn.Module):
    ...
    def forward(self, x_data, x_mask, y_data, y_mask):
        ...
        enc_out, enc_entropy_list, enc_attn_list= self.encoder(x_data, x_mask)
        ...
        dec_out, self_entropy_list, cross_entropy_list, self_attn_list, cross_attn_list = self.decoder(y_in, y_mask, enc_out, x_mask)
        ...
        return loss, enc_entropy_list, self_entropy_list, cross_entropy_list, enc_attn_list, self_attn_list, cross_attn_list
