...
def validation(model, x_d, x_m, y_d, y_m, args):
    ...
    loss1 = model(x_d, x_m, y_d, y_m) # 검증 데이터로 손실 계산

    return loss1.item()#, y_hat # 손실값 반환
...
def train_main(args):
    ...
    for x_data, x_mask, y_data, y_mask, cur_line, iloop in train_iter:
        ...
        if iloop % args.valid_interval == 0 and iloop >= args.valid_start: # # validation 시작 시점 확인 
            v_loss_sum = []  # validation loss를 저장할 리스트
            v_loss_avg = 0   # validation loss의 평균값
    
            # validation용 데이터 iterator 생성
            valid_iter = TextPairIterator(args.valid_src_file, args.valid_trg_file,
                                          args.src_dict, args.trg_dict,
            ...
            # validation 데이터에 대해 손실 계산
            for x_d, x_m, y_d, y_m, cur_l, il in valid_iter:
                vloss = validation(model, x_data, x_mask, y_data, y_mask, args)
        ...
        if iloop % args.log_interval == 0:
            ...
            if args.logging == 1:
                ...
                wandb.log({
                    ...
                    #**{f"encoder.attn_temp{i}": param[f'encoder.layer_stack.{i}.self_attn.sdp_attn.temper'].item() for i in range(6)},
                    #**{f"self_decoder.attn_temp{i}": param[f'decoder.layer_stack.{i}.self_attn.sdp_attn.temper'].item() for i in range(6)},
                    #**{f"cross_decoder.attn_temp{i}": param[f'decoder.layer_stack.{i}.enc_attn.sdp_attn.temper'].item() for i in range(6)},
