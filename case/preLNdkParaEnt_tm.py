...
from torch.nn.init import xavier_uniform_, xavier_normal_, constant_ # type: ignore
...
def calculate_entropy(attn, mask):
    ...
    # 엔트로피 계산: e = -entropy_param * attn * log(attn)
    entropy = -masked_attn * torch.log(masked_attn + 1e-9)

    # 차원 합산 및 마스크 적용
    entropy = entropy * mask  # 엔트로피에 마스크 적용
    entropy_sum = entropy.sum(dim=-1)  # Tk 차원 합산
    mask_sum = mask.sum(dim=-1) + 1e-9  # 마스크된 유효 값 개수

    # 마스크 기반 평균 계산: sum(e * m) / sum(m)
...
class Transformer(nn.Module):
    ...
    def forward(self, x_data, x_mask, y_data, y_mask):
        ...
        #encode and decode
        enc_out, enc_entropies = self.encoder(x_data, x_mask) # B Tx E
        dec_out, dec_self_entropies, dec_cross_entropies  = self.decoder(y_in, y_mask, enc_out, x_mask) # B Ty E
        ...
        return loss, enc_entropies, dec_self_entropies, dec_cross_entropies
