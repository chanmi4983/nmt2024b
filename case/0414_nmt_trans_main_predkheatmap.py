...
def plot_avg_attention_heatmap(layer_attn, tokens_src, tokens_tgt, layer_idx, save_dir, filename_prefix, is_self_attn=True):
    if isinstance(layer_attn, torch.Tensor):
        ...
        plt.title(
            f'{filename_prefix} Layer {layer_idx+1} - Average Attention',
...
def plot_all_layers_avg_attention(attn_list, tokens_src, tokens_tgt, save_dir, filename_prefix, is_self_attn=True):
    if all(isinstance(layer_attn, torch.Tensor) for layer_attn in attn_list):
        ...
        plt.yticks(rotation=0, fontsize=17)
        plt.title(
            f'{filename_prefix} - All Layers Average Attention',
            fontdict={'fontsize': 30, 'fontweight': 'bold'}
        )
        plt.tight_layout()
...
def translate_file(model, src_file, s_dict_file, trg_file, t_dict_file, trans_file, args, valid=None, save_attention=False, remove_bos=False):
    ...
    if save_attention:
        ...
        save_base_dir = os.path.join(os.path.dirname(trans_file), f"attention_maps_{attn_mode}_mid_sizeup")
        ...
        for i, (x_data, x_mask, cur_line, iloop) in enumerate(valid_iter):
            ...
            if args.model == 'tm':
                ...
                for layer_idx, layer_attn in enumerate(enc_attn_list):
                    ...
                    plot_avg_attention_heatmap(layer_attn, processed_src, processed_src, layer_idx, save_dir, "Encoder_self Attention", is_self_attn=True)

                plot_all_layers_avg_attention(enc_attn_list, processed_src, processed_src, save_dir, "Encoder_self Attention", is_self_attn=True)
