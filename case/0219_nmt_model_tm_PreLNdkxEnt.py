...
class ScaledDotProductAttention(nn.Module):
    def __init__(self, dk, drop_p=0.):
        ...
        self.temp_const = float(dk) # TEST it was better without 0.5 (layer_norm, init) # 논문과 확인
...
class TM_EncoderLayer(nn.Module):
    ...
    def forward(self, enc_in, x_mask=None):
        enc_out, enc_entropy = self.self_attn(enc_in, enc_in, enc_in, attn_mask=x_mask)
        enc_out = self.ff_layer(enc_out)
        return enc_out, enc_entropy
...
class TM_Encoder(nn.Module):
    ...
    def forward(self, src_seq, src_mask):
        ...
        for enc_layer in self.layer_stack:
            enc_out, enc_entropy = enc_layer(enc_out, x_mask=src_mask)
            enc_entropy_list.append(enc_entropy)
        return enc_out, enc_entropy_list
...
class TM_Decoder(nn.Module):
    ...
    def forward(self, trg_seq, trg_mask, enc_out, src_mask):
        ...
        for dec_layer in self.layer_stack:
            dec_out,self_entropy, cross_entropy = dec_layer(dec_out, enc_out, y_mask=y_mask, enc_mask=src_mask)
        ...
        return dec_out,self_entropy_list, cross_entropy_list
...
class Transformer(nn.Module):
    ...
    def forward(self, x_data, x_mask, y_data, y_mask):
        ...
        enc_out, enc_entropy_list = self.encoder(x_data, x_mask)
        ...
        dec_out, self_entropy_list, cross_entropy_list = self.decoder(y_in, y_mask, enc_out, x_mask)
        ...
        return loss, enc_entropy_list, self_entropy_list, cross_entropy_list
